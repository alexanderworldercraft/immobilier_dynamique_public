...
while($row = mysqli_fetch_row($result)) {
    ...
    echo '          <div class="d-flex flex-column justify-content-evenly align-items-center w-100 h-100 transition: 750ms;">';
...
<script>
    function applyBackgroundCarousel(element, imageUrls, idMaison) {
        ...
        function changeBackgroundImage() {
            element.style.backgroundImage = 'url(https://worldercraft-hebergement.web.app/immobilier/stockage/maison/' + idMaison + '/' + imageUrls[currentIndex] + '.light.png)';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        <?php for ($j = 0; $j < $i; $j++): ?>
            applyBackgroundCarousel(document.getElementById('carousel<?php echo $j; ?>'), <?php echo json_encode($imageUrls[$j]); ?>, '<?php echo $idMaisons[$j]; ?>');
        <?php endfor; ?>
    });
</script>
